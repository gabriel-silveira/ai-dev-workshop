...
1. Primeiro, garanta que todos os testes estão funcionando para a branch atual. Execute tanto os testes unitários quanto os testes E2E do projeto e confirme que todos passam. Se algum teste falhar, corrija os problemas antes de prosseguir.
...
4. Pergunte ao usuário qual estratégia de PR seguir:
   - **2 PRs (padrão)**: Um PR para `homol` (homologação) e outro para `main` (produção). Este é o fluxo padrão do projeto.
   - **Apenas homol**: Apenas um PR para `homol`, quando a feature ainda precisa de validação antes de ir para produção.

   Ambos os PRs devem ser criados a partir da mesma feature branch. NUNCA crie PR de `homol` para `main`.

5. Abra o(s) Pull Request(s) com os detalhes da implementação:

   Importante: Não mencione nenhum código relacionado a AI ou Claude no PR.

6. Para cada PR criado, aguarde 3 minutos e verifique comentários da ferramenta automatizada de code review. Se nenhum comentário aparecer, aguarde mais 3 minutos e verifique novamente.
...
8. Para os comentários que requerem correções:
   ...
   c. Faça push do(s) novo(s) commit(s) para a mesma branch (ambos os PRs serão atualizados automaticamente)
...
- PR(s) aberto(s): [INSERT PR TITLE]
- Comentários do code review automatizado abordados e correções pushed

O(s) PR(s) está(ão) agora pronto(s) para sua revisão final e merge manual.
